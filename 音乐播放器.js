...
const nextbtn = document.getElementById("next"),allMusic = document.querySelector(".allMusic"),returnbtn = document.querySelector('.returnRecent'),playMethod = document.querySelector('.playMethod');
...
getFile.onchange = function (e) {
    while (allMusic.firstElementChild.nextElementSibling.nextElementSibling) {
...
function defendTremble() {
    timer = setTimeout(function () {
        for (var k = 0; k < List.length; k++) {
            if (List[k].name.search(new RegExp(searchText.value.trim(), 'i')) != -1) {
                var li = document.createElement('li');
...
searchText.onkeyup = function () {
    //清除之前的查询记录
    var childs = searchSuggest.childNodes;
    for (var k1 = childs.length - 1; k1 >= 0; k1--) {
